...
class JamTreeNode {
    ...
    Map<AbstractAction, JamTreeNode> children = new HashMap<>();
    // Depth of this node
    final int depth;
    ...
    protected JamTreeNode(JamMCTSPlayer player, JamTreeNode parent, AbstractGameState state, Random rnd) {
        this.player = player;
        this.fmCallsCount = 0;
        this.parent = parent;
    ...
    void mctsSearch() {
        ...
        while (!stop) {
            ...
            JamTreeNode selected = treePolicy();
     ...
     */
    private JamTreeNode treePolicy() {

        JamTreeNode cur = this;
    ...
    private JamTreeNode expand() {
        ...
        JamTreeNode tn = new JamTreeNode(player, this, nextState, rnd);
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : children.keySet()) {
            JamTreeNode child = children.get(action);
            if (child == null)
    ...
    AbstractAction bestAction() {
        ...
        for (AbstractAction action : children.keySet()) {
            if (children.get(action) != null) {
                JamTreeNode node = children.get(action);
